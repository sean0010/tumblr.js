...
function requestCallback(callback) {
  ...
  return function (err, response, body) {
    ...
    var ratelimit = {};
    var ratelimit_remaining;
    for(var key in response.headers) {
      if(key.match(/x-ratelimit/)) {
        ratelimit[key] = response.headers[key];
        if(key.match(/remaining$/)) {
          if(ratelimit[key] <= 5) {
            console.warn("RATELIMIT HEADER WARNING: ", ratelimit);
          }
        }
      }
    }
/*
{ 'x-ratelimit-api_following-limit': '60',
  'x-ratelimit-api_following-remaining': '59',
  'x-ratelimit-api_following-reset': '1421227380' }
*/
    if (response.statusCode >= 400) {
      var err = body.meta ? body.meta.msg : body.error;
      return callback(new Error('API error: ' + response.statusCode + ' ' + err), '', ratelimit);
    }
    return callback(null, body.response, ratelimit);
  };
}
